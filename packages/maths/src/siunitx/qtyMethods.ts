...
import {
  convertToFixed,
  postProcessNumber,
} from './numPostProcessMethods';
import { IOptions, findOptions } from './options/options';
import {
  IQuantityOptions,
  PrefixMode,
  SeparateUncertaintyUnits,
} from './options/quantityOptions';
import { IUnitPiece, displayUnits, parseUnit } from './unitMethods';
import { prefixPower } from './units';

function combineExponent(
  parser: TexParser,
  num: INumberPiece,
  units: IUnitPiece[],
  ...
  if (firstUnit.prefix) {
    const unitPower =
      (firstUnit.power ? +firstUnit.power : 1) *
      (firstUnit.position === 'denominator' ? -1 : 1);
    const addedPower = firstUnit.prefix
      ? prefixPower.get(firstUnit.prefix)!
      : 1;
...
function extractExponent(
  ...
  for (let i = 0; i < units.length; i++) {
    ...
    if (
      (unit.symbol !== 'g' && unit.prefix !== '') ||
      (unit.symbol === 'g' &&
        unit.prefix !== '' &&
        !options['extract-mass-in-kilograms'])
    ) {
      const unitPower =
        (unit.power !== undefined && unit.power !== null
          ? +unit.power
          : 1) * (unit.position === 'denominator' ? -1 : 1);
      // if (binaryPrefixPower.has(unit.prefix)){
      // 	const prefPower = binaryPrefixPower.get(unit.prefix);
      // 	powersOfTwo += (prefPower*unitPower);
      // } else {
      if (unit.prefix && prefixPower.has(unit.prefix)) {
  ...
  const currentExponent = num.exponent
    ? +(num.exponentSign + num.exponent)
    : 0;
...
export const prefixModeMap = new Map<
  PrefixMode,
  (
    parser: TexParser,
    num: INumberPiece,
...
>([
  ...
  [
    ...
    (
      ...
      const root = parser.create('node', 'inferredMrow', [], {});
      let uncertaintyNode: MmlNode | undefined = undefined;
      ...
      if (uncertaintyNode) {
        const leftBracket = parser.create(
          'token',
          'mo',
          {},
          options['output-open-uncertainty'],
        );
        const rightBracket = parser.create(
          'token',
          'mo',
          {},
          options['output-close-uncertainty'],
        );
  ...
  [
    ...
    (
      ...
      if (uncertaintyNode) {
        ...
        const uncertaintyPosition =
          parent.childNodes.indexOf(uncertaintyNode);
        if (!quantityProduct) {
...
const separateUncertaintyUnitsMap = new Map<
  SeparateUncertaintyUnits,
  (num: string, units: string, options: IQuantityOptions) => string
>([
  [
    'single',
    (num: string, units: string, options: IQuantityOptions): string => {
...
export function createQuantityProductMml(
  parser: TexParser,
  options: IOptions,
): MmlNode | undefined {
  ...
  if (trimmedQuantityProduct) {
    const spacerNode = new TexParser(
...
export function createUnitsNode(
  unitLatex: string,
  parser: TexParser,
  options: IOptions,
): MmlNode {
  let unitNode = new TexParser(
    unitLatex,
    parser.stack.env,
    parser.configuration,
  ).mml();
...
export function processQuantity(parser: TexParser): void {
  ...
  if (globalOptions['parse-numbers']) {
    ...
    prefixModeMap.get(globalOptions['prefix-mode'])?.(
      parser,
      num,
      unitPieces,
      globalOptions,
    ...
    unitDisplay = displayUnits(
      parser,
      unitPieces,
      globalOptions,
      isLiteral,
    );
    const unitNode = new TexParser(
      unitDisplay,
      parser.stack.env,
      parser.configuration,
    ).mml();

    const quantityProductNode = createQuantityProductMml(
      parser,
      globalOptions,
    );
    if (quantityProductNode) {
      const qtyDisplay = separateUncertaintyUnitsMmlMap.get(
        globalOptions['separate-uncertainty-units'],
      )!(numDisplay, unitNode, quantityProductNode, parser, globalOptions);
    ...
    const qtyDisplay = separateUncertaintyUnitsMap.get(
      globalOptions['separate-uncertainty-units'],
    )!(numDisplay, unitDisplay, globalOptions);
    const qtyNode = new TexParser(
      qtyDisplay,
      parser.stack.env,
      parser.configuration,
    ).mml();
