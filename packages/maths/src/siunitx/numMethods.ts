...
export declare type CharNumFunction = (
  text: string,
  numPiece: INumberPiece,
) => void;
...
function getLastNumPiece(numPiece: INumberPiece): INumberPiece {
  if (
    numPiece.uncertainty.length > 0 &&
    !numPiece.uncertainty[numPiece.uncertainty.length - 1].completed
  ) {
...
function parseOpenUncertainty(_text: string, numPiece: INumberPiece) {
  const uncertainty: IUncertainty = {
    ...UncertaintyDefault,
    type: 'bracket',
  };
...
function parseCloseUncertainty(_text: string, numPiece: INumberPiece) {
  if (numPiece.uncertainty.length === 0) {
    throw new TexError(
      '50',
      "Trying to close an uncertainty that doesn't exist.",
    );
  }
  const uncertainty =
    numPiece.uncertainty[numPiece.uncertainty.length - 1];
...
function parseUncertaintySigns(_text: string, numPiece: INumberPiece) {
...
export function generateNumberMapping(
  options: INumParseOptions,
): Map<string, CharNumFunction> {
  const parseMap = new Map<string, CharNumFunction>();
  const matchMacrosOrChar =
    /\\(?:[a-zA-Z]+|[\uD800-\uDBFF].|.)|[\uD800-\uDBFF].|[^\s\\]/g;
  for (const [key, method] of [
    ...
    const option = options[key as keyof INumParseOptions] as string;
    if (option.match(/(?:^|[^\\])(?:\\\\)*\\$/)) {
    ...
    (option.match(matchMacrosOrChar) || []).forEach((c: string) => {
      if (parseMap.has(c) && key === 'input-uncertainty-signs') {
        ...
        const altMethod: CharNumFunction = function (macro, num) {
          (num.whole === '' && num.decimal === ''
            ? inputSigns
            : parseUncertaintySigns)(macro, num);
        };
...
export function parseNumber(
  ...
  if (
    !options['retain-explicit-decimal-marker'] &&
    num.decimal &&
    !num.fractional
  ) {
    num.decimal = '';
  ...
  const value = +(
    ...
    (num.decimal ? '.' : '') +
    num.fractional +
    (num.exponent === '' ? '' : 'e' + num.exponentSign + num.exponent)
  );
  if (
    value === 0 &&
    !options['retain-negative-zero'] &&
    num.sign === '-'
  ) {
    num.sign = '';
  }
  ...
  if (!options['retain-zero-uncertainty']) {
    for (let i = num.uncertainty.length - 1; i >= 0; i--) {
      const uncertaintyValue = +(
        num.uncertainty[i].whole +
        (num.uncertainty[i].decimal ? '.' : '') +
        num.uncertainty[i].fractional
      );
