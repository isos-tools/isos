...
function processUnitMacro(
  ...
  const userDefinedUnits = parser.configuration.packageData.get('siunitx')[
    UserDefinedUnitsKey
  ] as Map<string, string>;
  if (userDefinedUnits.has('\\' + macro)) {
    const result = userDefinedUnits.get('\\' + macro);
    const userDefinedUnitOptions = parser.configuration.packageData.get(
      'siunitx',
    )[UserDefinedUnitOptionsKey] as Map<string, Partial<IOptions>>;
    const options = userDefinedUnitOptions.get('\\' + macro);
    return {
      type: 'unit',
      result: { symbol: result as string, prefix: '' },
      options: options,
    };
  }
...
>([
  ...
  [
    'tothe',
    (_macro: string, parser: TexParser): IUnitMacroProcessResult => {
...
const qualiferMethod = new Map<
  QualifierMode,
  (qualifer: string, phrase?: string) => string
>([
  [
    'subscript',
    (qualifer: string): string => {
      return '_{' + qualifer + '}';
    },
  ],
  [
...
function unitLatex(
  ...
  if (unitPiece.qualifier) {
    unitLatex += qualiferMethod.get(options['qualifier-mode'])?.(
      unitPiece.qualifier,
  ...
  unitLatex += '}';
  const power =
    unitPiece.power !== undefined && unitPiece.power !== null
      ? absPower
        ? Math.abs(
            unitPiece.power *
              (unitPiece.position === 'denominator' ? -1 : 1),
          )
...
export function displayUnits(
  ...
  if (
    ...
    unitPieces.filter((v) => {
      const power =
        v.power !== null && v.power !== undefined
          ? v.power * (v.position === 'denominator' ? -1 : 1)
          : 1;
      return Math.sign(power) === -1;
    }).length === 1 &&
    options['per-mode'] === 'single-symbol'
  ) {
    perForSingle = true;
  }
  ...
  if (isLiteral) {
    let latex = '';
    let startsSlash: IUnitPiece | undefined = undefined;
    unitPieces.every((p) => {
    ...
    unitPieces.forEach((v) => {
      if (
        v.position === 'denominator' ||
        (v.power !== null && v.power !== undefined && v.power < 0)
      ) {
        denominatorCount++;
      } else {
        numeratorCount++;
    ...
    if (
      options['per-mode'] === 'fraction' ||
      options['per-mode'] === 'symbol' ||
      options['per-mode'] === 'repeated-symbol' ||
      perForSingle ||
      (options['per-mode'] === 'single-symbol' &&
        denominatorCount === 1 &&
        numeratorCount > 0)
      ...
      if (denominator !== '') {
        ...
        if (options['per-mode'] === 'fraction') {
          ...
          throw siunitxError.DenominatorParsingError(
            denominator,
            options['per-mode'],
          );
        }
      ...
      switch (processedMacro.type) {
        ...
        case 'prefix':
          if (nextModifier !== null) {
            nextModifier = Object.assign(
              nextModifier,
              processedMacro.result,
            );
          } else {
        ...
        case 'unit': {
          if (nextModifier !== null) {
            processedMacro.result = Object.assign(
              processedMacro.result,
              nextModifier,
            );
            ...
            if (
              (parser.options.siunitx as IOptions)['per-mode'] ===
                'repeated-symbol' ||
              globalOptions['sticky-per']
            ) {
...
function processPrefixUnitCombo(
  text: string,
  unitPiece: IUnitPiece,
): void {
