...
export function processQuantityRange(parser: TexParser): void {
  ...
  if (globalOptions['parse-numbers']) {
    ...
    const exponentMapItem = exponentListModeMap.get(
      globalOptions['range-exponents'],
    )!;
    const exponentResult = exponentMapItem(
      [firstNum, lastNum],
      parser,
      globalOptions,
    );
    const unitsMapItem = unitListModeMap.get(
      globalOptions['range-units'],
    )!;
    ...
    const separator = new TexParser(
      `\\text{${globalOptions['range-phrase']}}`,
      parser.stack.env,
