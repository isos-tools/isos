...
import {
  Configuration,
  ParserConfiguration,
} from '@mathjax/src/js/input/tex/Configuration.js';
...
import { Token as TexToken } from '@mathjax/src/js/input/tex/Token.js';
import {
  CharacterMap,
  CommandMap,
...
import {
  processComplexNumber,
  processComplexQuantity,
} from './complexMethods';
...
import { StackItem } from '@mathjax/src/js/input/tex/StackItem.js';
...
function angleChars(
  parser: TexParser,
  mchar: string | TexToken | StackItem,
) {
  if (mchar && mchar instanceof TexToken) {
...
new CommandMap(
  ...
  },
  {
    siunitxToken: (
      parser: TexParser,
      name: string | TexToken | StackItem,
    ) => {
      if (typeof name === 'string') methodMap[name as string]?.(parser);
    },
    Arialabel: (
      parser: TexParser,
      name: string | TexToken | StackItem,
    ...
    // currently not used
    Dataset: (parser: TexParser, name: string | TexToken | StackItem) => {
      if (typeof name === 'string') {
