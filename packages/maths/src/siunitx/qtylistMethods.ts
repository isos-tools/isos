...
function singleExponent(
  exponentResult: IExponentModeOutput,
  unitLatex: string,
  parser: TexParser,
  options: IOptions,
): IUnitsModeOutput {
  ...
  if (exponentResult.trailing) {
    ...
    exponentResult.trailing = parser.create(
      'node',
      'inferredMrow',
      [],
      {},
  ...
  return {
    numbers: numNodes,
    leading: exponentResult.leading,
    trailing: exponentResult.trailing,
  };
}

function bracketExponent(
  exponentResult: IExponentModeOutput,
  unitLatex: string,
  parser: TexParser,
  options: IOptions,
  ...
  if (!exponentResult.leading) {
    ...
    const leadingBracket = new TexParser(
      bracketOpenMap.get(parser.currentCS)!(options),
      parser.stack.env,
  ...
  if (options['list-exponents'] !== 'combine-bracket') {
    const trailingBracket = new TexParser(
      bracketCloseMap.get(parser.currentCS)!(options),
      parser.stack.env,
      parser.configuration,
    ).mml();
...
export const unitListModeMap = new Map<
  UnitsModeProduct,
  (
    exponentResult: IExponentModeOutput,
    unitLatex: string,
...
>([
  ...
  [
    ...
    (
      ...
      const separator = new TexParser(
        `\\text{${options['list-pair-separator']}}`,
        parser.stack.env,
        parser.configuration,
      ).mml();
  ...
  [
    ...
    (
      ...
      const finalSeparator = new TexParser(
        `\\text{${options['list-final-separator']}}`,
        parser.stack.env,
        parser.configuration,
      ).mml();
      root.appendChild(finalSeparator);
      root.appendChild(
        unitsResult.numbers[unitsResult.numbers.length - 1],
      );
...
export function processQuantityList(parser: TexParser): void {
  ...
  if (globalOptions['parse-numbers']) {
    ...
    if (globalOptions['list-exponents'] === 'individual') {
      ...
      const altOptions = Object.assign(globalOptions, {
        exponentMode: 'fixed',
        fixedExponent: targetExponent,
      });
      numlist.forEach((v, i) => {
        if (i === 0) {
          postProcessNumber(parser, v, globalOptions);
    ...
    const mapItem =
      listNumberMap.get(numlist.length)! ?? listNumberMap.get(3)!;
